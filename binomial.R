...
set.seed(2)
S <- matrix(0, nrow = J + 1, ncol = k)
S[1, ] <- S0

# Simulate stock price paths
for (j in 1:k) {
  # Generate random upward and downward movements for each path
  steps <- sample(c(u, d), J, replace = TRUE, prob = c(p, 1 - p))
  S[, j] <- cumprod(c(S0, steps))
}
S <- t(S)
...
asset_prices <- matrix(0, nrow = J + 1, ncol = J + 1)
for (i in 0:J) {
  for (j in 0:i) {
    asset_prices[j + 1, i + 1] <- S0 * u^j * d^(i - j)
  }
}

exercise_dates <- seq(1/J, T, by = 1/J)  # Exercise dates (e.g., every 0.2 years)

# Initialize the option value tree
option_values <- matrix(0, nrow = J + 1, ncol = J + 1)
for (j in 0:J) {
  option_values[j + 1, J + 1] <- max(0, K - asset_prices[j + 1, J + 1])
}

# Traverse the tree backwards
for (i in (J - 1):0) {
  for (j in 0:i) {
    continuation_value <- discount * (p * option_values[j + 2, i + 2] +
                                        (1 - p) * option_values[j + 1, i + 2])
    exercise_value <- max(0, K - asset_prices[j + 1, i + 1])
    
    # Allow exercise only if on an exercise date
    if (i * dt %in% exercise_dates) {
      option_values[j + 1, i + 1] <- max(continuation_value, exercise_value)
    } else {
      option_values[j + 1, i + 1] <- continuation_value
    }
  }
}

bermudan_put_price <- option_values[1, 1]
cat("Bermudan Put Option Price (Binomial Model):", bermudan_put_price, "\n")
